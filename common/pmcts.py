...
class Parallel_MCTS:
    ...
    def parallel_pass(self, boards: List[BlankBoard]):
        ...
        '''
        tensors = torch.stack([board.to_tensor() if board is not None else torch.randn(self.board_shape) for board in boards], dim=0).to(device)
    ...
    def play(self):
        ...
        for _ in range(self.max_runs):
            ...
            for i, runs_left in enumerate(self.runs):
                if runs_left == 0:
                    values = np.array(
                        [-node.value_score() if node is not None else -np.inf for node in self.trees[i].children]
                    )
                    es = np.exp(values)
                    values = es / sum(es)
