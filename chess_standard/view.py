...
fen = "8/2p5/3p3k/KP5r/1R3p2/8/4P1P1/8 w - - 0 1"
        ...
        if button == arcade.MOUSE_BUTTON_LEFT and self.board.no_moves_left == False:
            ...
            if self.awaiting_promotion_selection:
                ...
                if self.is_mouse_hovering_promotion_sprite(
                    ...
                    knight_sprite, x, y
                ):  # CLICKED TYPE KNIGHT = 1
                    encoded_move[2:4] = int2ba(1, 2, endian="big")
                elif self.is_mouse_hovering_promotion_sprite(
                    bishop_sprite, x, y
                ):  # CLICKED TYPE BISHOP = 2
                    encoded_move[2:4] = int2ba(2, 2, endian="big")
                elif self.is_mouse_hovering_promotion_sprite(
                    queen_sprite, x, y
                ):  # CLICKED TYPE QUEEN = 3
                    encoded_move[2:4] = int2ba(3, 2, endian="big")

                encoded_move[4:10] = pos_idx_to_bitarray(
                    filerank_to_pos_idx(self.target_square), length=6
                )
                encoded_move[10:16] = pos_idx_to_bitarray(
                    filerank_to_pos_idx(self.origin_square), length=6
                )
                ...
                if self.board.is_piece_selectable(filerank):
                    ...
                    # else, assign the filerank and its valid moves to the class variables
                        ...
                        self.piece_type = self.board.get_piece_of_square(
                            filerank_to_pos_idx(filerank)
                        )
                    ...
                    if self.origin_square and filerank in self.valid_moves.keys():
                        ...
                        if self.awaiting_promotion_selection == False:
                            ...
                            self.game_state_array = (
                                self.board.visualize_current_gamestate()
                            )
                            self.piece_sprites.clear()
                            self.load_pieces()
