...
def single_perft(board_mine, board_engine, max_depth):
    ...
    table.add_row(['mine', max_depth, node_count_mine, node_count_mine, f'{elapsed_time_mine:.4f}'])
    table.add_row(['engine', max_depth, node_count_engine, node_count_engine, f'{elapsed_time_engine:.4f}'])
...
def table_perft(board_mine, board_engine, max_depth):
    ...
    for depth, nodes in counter.items():
        node_count_mine, total_nodes_mine, elapsed_time_mine, node_count_engine, total_nodes_engine, elapsed_time_engine = nodes
        table.add_row(['mine', depth, node_count_mine, total_nodes_mine, f'{elapsed_time_mine:.4f}'])
        table.add_row(['engine', depth, node_count_engine, total_nodes_engine, f'{elapsed_time_engine:.4f}'], divider=True)
...
def perft_divide(board, depth):
    ...
    for move in legal_moves:
        ...
        # MOVE STRING
        if isinstance(board, BoardChess):
            origin_square, target_square, promotion_piece_type, special_move_flag = decode_move(move)
    ...
    for i, fen in enumerate(fen_strings):
        ...
        for move in moves:
            ...
            if value1 == value2:
                ...
                same = False
                print(f"{red_start}{move}:\t{value1}\t{value2}\t{value1 - value2}{red_end}")
        ...
        print(f'Times:\t{(end_mine - start_mine):.3f}\t{(end_engine - start_engine):.3f}')
