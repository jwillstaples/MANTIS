...
class BoardChess(BlankBoard):
    ...
    def __init__(self, white_move=1):
        ...
        self.castling_rights = ones(
            4, endian="little"
        )  # [White Kingside, White Queenside, Black Kingside, Black Queenside], 1 = True
    ...
    def make_move(self, encoded_move):
        ...
        if self.white_move == 1:
            ...
            if (piece_type == "P") and (
                ...
                en_passant_target_filerank = (
                    en_passant_target_file + en_passant_target_rank
                )
                self.en_passant_target_pos_idx = filerank_to_pos_idx(
                    en_passant_target_filerank
                )
    ...
    def visualize_current_gamestate(self):
        ...
        print(
            f"halfmove_counter = {self.halfmove_counter} | fullmove_counter = {self.fullmove_counter}"
        )
        print(
            f"en_passant_target_pos_idx = {self.en_passant_target_pos_idx} | {en_passant_filerank}"
        )
    ...
    def get_attack_ray_of_target_square(self, target_square_pos_idx, input_type):
        ...
        if input_type == "rook":
            for dir4 in [0, 1, 2, 3]:
                rook_attacks = getRookRayAttacks(
                    self.rook_rays, occupied_minus_king, dir4, target_square_pos_idx
                )
                attack_ray |= rook_attacks

        elif input_type == "bishop":
            for dir4 in [0, 1, 2, 3]:
                bishop_attacks = getBishopRayAttacks(
                    self.bishop_rays, occupied_minus_king, dir4, target_square_pos_idx
                )
                attack_ray |= bishop_attacks

        elif input_type == "queen":
            for dir4 in [0, 1, 2, 3]:
                line_attacks = getRookRayAttacks(
                    self.rook_rays, occupied_minus_king, dir4, target_square_pos_idx
                )
                diag_attacks = getBishopRayAttacks(
                    self.bishop_rays, occupied_minus_king, dir4, target_square_pos_idx
                )
    ...
    def get_pinned_piece_dict(self, king_pos):
        ...
        for dir4 in [0, 1, 2, 3]:
            ...
            king_rook_attack_rays[dir4] = getRookRayAttacks(
                self.rook_rays, self.occupied_squares, dir4, king_pos
            )
            # 0 = NORTHWEST, 1 = NORTHEAST, 2 = SOUTHEAST, 3 = SOUTHWEST
            king_bishop_attack_rays[dir4] = getBishopRayAttacks(
                self.bishop_rays, self.occupied_squares, dir4, king_pos
            )
            ...
            for pinning_attackers in ["r", "b", "q"]:
                for pidx in self.black_piece_bitboards[pinning_attackers].search(1):
                    ...
                    if pinning_attackers == "r":
                        for dir4 in [0, 1, 2, 3]:
                            # 0 = NORTH, 1 = EAST, 2 = SOUTH, 3 = WEST
                            attacker_ray[dir4] = getRookRayAttacks(
                                self.rook_rays, self.occupied_squares, dir4, pidx
                            )

                        king_pinned_from_northern_rook = (
                            king_rook_attack_rays[0] & attacker_ray[2]
                        )  # use rook's southern ray
                        king_pinned_from_eastern_rook = (
                            king_rook_attack_rays[1] & attacker_ray[3]
                        )  # use rook's western ray
                        king_pinned_from_southern_rook = (
                            king_rook_attack_rays[2] & attacker_ray[0]
                        )  # use rook's northern ray
                        king_pinned_from_western_rook = (
                            king_rook_attack_rays[3] & attacker_ray[1]
                        )  # use rook's eastern ray
                        ...
                        for dir4 in [0, 1, 2, 3]:
                            # 0 = NORTHWEST, 1 = NORTHEAST, 2 = SOUTHEAST, 3 = SOUTHWEST
                            attacker_ray[dir4] = getBishopRayAttacks(
                                self.bishop_rays, self.occupied_squares, dir4, pidx
                            )

                        king_pinned_from_NW_bishop = (
                            king_bishop_attack_rays[0] & attacker_ray[2]
                        )  # use bishop's SE ray
                        king_pinned_from_NE_bishop = (
                            king_bishop_attack_rays[1] & attacker_ray[3]
                        )  # use bishop's SW ray
                        king_pinned_from_SE_bishop = (
                            king_bishop_attack_rays[2] & attacker_ray[0]
                        )  # use bishop's NW ray
                        king_pinned_from_SW_bishop = (
                            king_bishop_attack_rays[3] & attacker_ray[1]
                        )  # use bishop's NE ray
                        ...
                        king_pinned_from_N_queen = (
                            king_queen_attack_rays[0] & attacker_ray[2]
                        )
                        king_pinned_from_E_queen = (
                            king_queen_attack_rays[1] & attacker_ray[3]
                        )
                        king_pinned_from_S_queen = (
                            king_queen_attack_rays[2] & attacker_ray[0]
                        )
                        king_pinned_from_W_queen = (
                            king_queen_attack_rays[3] & attacker_ray[1]
                        )
                        king_pinned_from_NW_queen = (
                            king_queen_attack_rays[4] & attacker_ray[6]
                        )
                        king_pinned_from_NE_queen = (
                            king_queen_attack_rays[5] & attacker_ray[7]
                        )
                        king_pinned_from_SE_queen = (
                            king_queen_attack_rays[6] & attacker_ray[4]
                        )
                        king_pinned_from_SW_queen = (
                            king_queen_attack_rays[7] & attacker_ray[5]
                        )
    ...
    def get_attackers_of_target_square(self, target_square_pos_idx):
        ...
        """
        square_rook_attack_ray = self.get_attack_ray_of_target_square(
            target_square_pos_idx, input_type="rook"
        )
        square_bishop_attack_ray = self.get_attack_ray_of_target_square(
            target_square_pos_idx, input_type="bishop"
        )
        square_queen_attack_ray = self.get_attack_ray_of_target_square(
            target_square_pos_idx, input_type="queen"
        )
        square_knight_attack_ray = self.get_attack_ray_of_target_square(
            target_square_pos_idx, input_type="knight"
        )
        square_king_attack_ray = self.get_attack_ray_of_target_square(
            target_square_pos_idx, input_type="king"
        )

        if self.white_move == 1:
            square_pawn_attack_ray = self.get_attack_ray_of_target_square(
                target_square_pos_idx, input_type="white_pawn"
            )
            ...
            square_pawn_attack_ray = self.get_attack_ray_of_target_square(
                target_square_pos_idx, input_type="black_pawn"
            )
        ...
        if piece_type in ["R", "B", "Q", "r", "b", "q"]:
            ...
            if piece_type == "R" or piece_type == "r":
                if file_o == file_t and int(rank_o) < int(rank_t):  # NORTH RAY
                    mask = getRookRayAttacks(
                        self.rook_rays, occupied_plus_target, 0, origin_square
                    )
                    exclusive_inclusive_ray = mask & self.file[file_o]
                elif (
                    files.index(file_o) < files.index(file_t) and rank_o == rank_t
                ):  # EAST RAY
                    mask = getRookRayAttacks(
                        self.rook_rays, occupied_plus_target, 1, origin_square
                    )
                    exclusive_inclusive_ray = mask & self.rank[rank_o]
                elif file_o == file_t and int(rank_o) > int(rank_t):  # SOUTH RAY
                    mask = getRookRayAttacks(
                        self.rook_rays, occupied_plus_target, 2, origin_square
                    )
                    exclusive_inclusive_ray = mask & self.file[file_o]
                elif (
                    files.index(file_o) > files.index(file_t) and rank_o == rank_t
                    ...
                    mask = getRookRayAttacks(
                        self.rook_rays, occupied_plus_target, 3, origin_square
                    )
                    exclusive_inclusive_ray = mask & self.rank[rank_o]

            elif piece_type == "B" or piece_type == "b":
                # 0 = NORTHWEST, 1 = NORTHEAST, 2 = SOUTHEAST, 3 = SOUTHWEST
                if files.index(file_o) > files.index(file_t) and int(rank_o) < int(
                    rank_t
                ):  # NORTHWEST RAY
                    mask = getBishopRayAttacks(
                        self.bishop_rays, occupied_plus_target, 0, origin_square
                    )
                    exclusive_inclusive_ray = mask
                elif files.index(file_o) < files.index(file_t) and int(rank_o) < int(
                    rank_t
                ):  # NORTHEAST RAY
                    mask = getBishopRayAttacks(
                        self.bishop_rays, occupied_plus_target, 1, origin_square
                    )
                    exclusive_inclusive_ray = mask
                elif files.index(file_o) < files.index(file_t) and int(rank_o) > int(
                    rank_t
                    ...
                    mask = getBishopRayAttacks(
                        self.bishop_rays, occupied_plus_target, 2, origin_square
                    )
                    exclusive_inclusive_ray = mask
                elif files.index(file_o) > files.index(file_t) and int(rank_o) > int(
                    rank_t
                    ...
                    mask = getBishopRayAttacks(
                        self.bishop_rays, occupied_plus_target, 3, origin_square
                    )
                    exclusive_inclusive_ray = mask
    ...
    def get_legal_moves(self):
        ...
        if self.white_move == 1:
            ...
            enemy_attackers_of_king = self.get_attackers_of_target_square(
                king_pos
            )  # enemy = for check computing
            for pidx in enemy_attackers_of_king.search(1):
                if self.white_pieces[pidx] == 1:
                    enemy_attackers_of_king[pidx] = 0
        elif self.white_move == -1:
                ...
                if special_move_flag == 3:  # CASTLING KING MOVES LEGAL CHECK
                    ...
                    if num_attackers == 0:
                        if target_square > origin_square:  # KINGSIDE CASTLING
                            ...
                            for castling_square in range(
                                origin_square + 1, target_square + 1
                            ):
                                attackers = self.get_attackers_of_target_square(
                                    castling_square
                                )

                                if (
                                    self.white_move == 1
                                ):  # filter out friendly "attackers"
                            ...
                            for castling_square in range(
                                origin_square - 1, target_square - 1, -1
                            ):
                                attackers = self.get_attackers_of_target_square(
                                    castling_square
                                )
                    ...
                    if (
                        not attackers.any()
                    ):  # there does not exist a piece that attacks target_square
                        legal_encoded_moves.append(
                            encoded_move
                        )  # i.e., can move the King there

            # ANY OTHER PIECE MOVES
            else:
                if (
                    self.get_piece_of_square(target_square) == "K"
                    or self.get_piece_of_square(target_square) == "k"
                ):
                    continue
                    ...
                    if num_attackers >= 2:  # DOUBLE CHECK
                        ...
                        attacker_ray_bitboard = self.get_between_source_target_ray(
                            attacker_pos_idx, king_pos
                        )  # exclusive, inclusive
                        ...
                        if (
                            pinned_piece_bitboard[origin_square] == 0
                        ):  # two conditions for legality IF the moving piece isn't a pinned piece
                            if (
                                target_square == attacker_pos_idx
                            ):  # pseudolegal = legal if can capture, or
                                legal_encoded_moves.append(encoded_move)
                            elif (
                                attacker_ray_bitboard[target_square] == 1
                            ):  # we can move it to block the attacker
                                legal_encoded_moves.append(
                                    encoded_move
                                )  # if target_sq in the attacker_ray, it's blocking
                        else:  # if it is a pinned piece, gg just skip
                            continue

                    else:  # NOT IN CHECK, need to check if moving piece is absolutely pinned
                        if (
                            pinned_piece_bitboard[origin_square] == 1
                        ):  # if a pinned piece, can only move along the pinned ray
                            attacker_pos_idx = pinned_piece_dict[
                                origin_square
                            ]  # get idx of the attacking piece that is pinning the move piece
                            attacker_ray_bitboard = self.get_between_source_target_ray(
                                attacker_pos_idx, king_pos
                            )
                            # lerf_bitboard_to_2D_numpy(attacker_ray_bitboard)
                            ...
                            if target_square == attacker_pos_idx:
                                ...
                                attacker_ray_bitboard[target_square] == 1
                            ):  # if the move is within the ray, it's legal, otherwise not legal
                                legal_encoded_moves.append(encoded_move)
                            ...
                            if special_move_flag == 2:
                                ...
                                if self.white_move == 1:
                                    ...
                                    enemy_attackers_of_king = (
                                        self.get_attackers_of_target_square(king_pos)
                                    )  # enemy = for check computing
                                    for pidx in enemy_attackers_of_king.search(1):
                                        if self.white_pieces[pidx] == 1:
                                            enemy_attackers_of_king[pidx] = 0
    ...
    def generate_pseudolegal_queen_moves(self):
        ...
        if self.white_move == 1:
            for pos_idx in self.white_queens.search(1):
                ...
                for dir4 in [0, 1, 2, 3]:
                    line_attacks = getRookRayAttacks(
                        self.rook_rays, self.occupied_squares, dir4, pos_idx
                    )
                    diag_attacks = getBishopRayAttacks(
                        self.bishop_rays, self.occupied_squares, dir4, pos_idx
                    )
                    queen_attacks = line_attacks | diag_attacks
                    ...
                    for target_pos_idx in queen_attacks.search(1):
                        ...
                        target_squares.append(
                            pos_idx_to_bitarray(target_pos_idx, length=6)
                        )
    ...
    def generate_pseudolegal_king_moves(self):
        ...
        if self.white_move == 1:
            for pos_idx in self.white_king.search(1):
                ...
                if (not self.rank["8"][pos_idx]) and bool(  # north
                    (self.empty_squares[pos_idx - 8] or self.black_pieces[pos_idx - 8])
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx - 8, length=6))
                if (  # northeast
                    (not self.rank["8"][pos_idx])
                    and (not self.file["h"][pos_idx])
                    and bool(
                        (
                            self.empty_squares[pos_idx - 7]
                            or self.black_pieces[pos_idx - 7]
                        )
                    )
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx - 7, length=6))
                if (not self.file["h"][pos_idx]) and bool(  # east
                    (self.empty_squares[pos_idx + 1] or self.black_pieces[pos_idx + 1])
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx + 1, length=6))
                if (  # southeast
                    (not self.rank["1"][pos_idx])
                    and (not self.file["h"][pos_idx])
                    and bool(
                        (
                            self.empty_squares[pos_idx + 9]
                            or self.black_pieces[pos_idx + 9]
                        )
                    )
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx + 9, length=6))
                if (not self.rank["1"][pos_idx]) and bool(  # south
                    (self.empty_squares[pos_idx + 8] or self.black_pieces[pos_idx + 8])
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx + 8, length=6))
                if (  # southwest
                    (not self.rank["1"][pos_idx])
                    and (not self.file["a"][pos_idx])
                    and bool(
                        (
                            self.empty_squares[pos_idx + 7]
                            or self.black_pieces[pos_idx + 7]
                    ...
                    target_squares.append(pos_idx_to_bitarray(pos_idx + 7, length=6))
                if (not self.file["a"][pos_idx]) and bool(  # west
                    (self.empty_squares[pos_idx - 1] or self.black_pieces[pos_idx - 1])
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx - 1, length=6))
                if (  # northwest
                ...
                # WHITE KINGSIDE
                if (
                    (self.castling_rights[0] == 1)
                    and (  # white king and kingside white rook have not moved
                        self.white_rooks[7 ^ 56] == 1
                    )
                    and bool(
                        (
                            self.empty_squares[pos_idx + 1]
                            and self.empty_squares[pos_idx + 2]
                        )
                    )  # empty squares between them
                ):
                    encoded_move = zeros(16, endian="big")
                ...
                if (
                    ...
                    and (self.white_rooks[0 ^ 56] == 1)
                    and bool(
                        (
                            self.empty_squares[pos_idx - 1]
                            and self.empty_squares[pos_idx - 2]
                            and self.empty_squares[pos_idx - 3]
                        )
                    )
                ):
                    encoded_move = zeros(16, endian="big")
            ...
            for pos_idx in self.black_king.search(1):
                ...
                if (not self.rank["8"][pos_idx]) and bool(  # north
                    (self.empty_squares[pos_idx - 8] or self.white_pieces[pos_idx - 8])
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx - 8, length=6))
                if (  # northeast
                    (not self.rank["8"][pos_idx])
                    ...
                    and bool(
                        (
                            self.empty_squares[pos_idx - 7]
                            or self.white_pieces[pos_idx - 7]
                        )
                    )
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx - 7, length=6))
                if (not self.file["h"][pos_idx]) and bool(  # east
                    (self.empty_squares[pos_idx + 1] or self.white_pieces[pos_idx + 1])
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx + 1, length=6))
                if (  # southeast
                    (not self.rank["1"][pos_idx])
                    ...
                    and bool(
                        (
                            self.empty_squares[pos_idx + 9]
                            or self.white_pieces[pos_idx + 9]
                        )
                    )
                ):
                    target_squares.append(pos_idx_to_bitarray(pos_idx + 9, length=6))
                ...
                # BLACK KINGSIDE
                if (
                    (self.castling_rights[2] == 1)
                    and (  # black king and kingside white rook have not moved
                        self.black_rooks[63 ^ 56] == 1
                    )
                    and bool(
                        (self.empty_squares[pos_idx + 1])
                        and self.empty_squares[pos_idx + 2]
                ...
                # BLACK QUEENSIDE
                if (
                    (self.castling_rights[3] == 1)
                    and (self.black_rooks[56 ^ 56] == 1)
                    and bool(
                        (
                            self.empty_squares[pos_idx - 1]
                            and self.empty_squares[pos_idx - 2]
        ...
        if self.white_move == 1:
            for pos_idx in self.white_knights.search(1):
                ...
                for shift in [6, 15, 17, 10]:
                    wraparound_file_north, wraparound_file_south = wraparound_masks[
                        shift
                    ]
                    ...
                    if (
                        (pos_idx - shift >= 0)
                        and (  # handles going out of range
                            not wraparound_file_north[pos_idx]
                        )
                        and bool(  # handles wraparound
                            (
                                self.empty_squares[pos_idx - shift]
                                or self.black_pieces[pos_idx - shift]
                            )
                        )  # handles valid move
                    ):
                        target_squares.append(
                            pos_idx_to_bitarray(pos_idx - shift, length=6)
                        )

                    # SOUTH LOGIC
                    if (
                        (pos_idx + shift <= 63)
                        and (not wraparound_file_south[pos_idx])
                        and bool(
                            (
                                self.empty_squares[pos_idx + shift]
                                or self.black_pieces[pos_idx + shift]
            ...
            for pos_idx in self.black_knights.search(1):
                ...
                for shift in [6, 15, 17, 10]:
                    ...
                    if (
                        (pos_idx + shift <= 63)
                        and (not wraparound_file_north[pos_idx])
                        and bool(
                            (
                                self.empty_squares[pos_idx + shift]
                                or self.white_pieces[pos_idx + shift]
                            )
                        )
                    ):
                        target_squares.append(
        ...
        if self.white_move == 1:
            for pos_idx in self.white_pawns.search(1):
                ...
                if ~self.rank["8"][pos_idx]:
                    ...
                    if self.empty_squares[pos_idx - 8]:  # SINGLE STEP
                        target_squares.append(
                            pos_idx_to_bitarray(pos_idx - 8, length=6)
                        )
                    if self.rank["2"][pos_idx]:
                        if (
                            self.empty_squares[pos_idx - 8]
                            and self.empty_squares[pos_idx - 16]
                        ):  # DOUBLE STEP
                            target_squares.append(
                                pos_idx_to_bitarray(pos_idx - 16, length=6)
                            )

                    # CAPTURE MOVE LOGIC
                    if (
                        not self.file["a"][pos_idx] and self.black_pieces[pos_idx - 9]
                    ):  # LEFT CAPTURE
                        target_squares.append(
                            pos_idx_to_bitarray(pos_idx - 9, length=6)
                        )
                    if (
                        not self.file["h"][pos_idx] and self.black_pieces[pos_idx - 7]
                    ):  # RIGHT CAPTURE
                        target_squares.append(
                            pos_idx_to_bitarray(pos_idx - 7, length=6)
                        )

                # EN PASSANT LOGIC, en_passant_target_pos_idx = en passant target square of black pawn
                if self.en_passant_target_pos_idx != -1:
                    filerank_target = pos_idx_to_filerank(
                        self.en_passant_target_pos_idx
                    )
                    file_t, rank_t = filerank_target[0], filerank_target[1]

                    is_adjacent = abs(files.index(file) - files.index(file_t)) == 1

                    if (int(rank) == (int(rank_t) - 1)) and (  # check for same rank
                        is_adjacent == True
                    ):  # check for adjacent file
                        encoded_move = zeros(16, endian="big")
                        encoded_move[0:2] = int2ba(
                            2, 2, endian="big"
                        )  # EN PASSANT FLAG = 10
                        encoded_move[4:10] = pos_idx_to_bitarray(
                            self.en_passant_target_pos_idx, length=6
                        )
                        encoded_move[10:16] = origin_square
                ...
                for target_square in target_squares:
                    ...
                    if bitarray_to_pos_idx(target_square) in range(0, 8):
                        for i in range(0, 4):  # Loop over promotion piece types
                            ...
                            encoded_move[2:4] = int2ba(
                                i, 2, endian="big"
                            )  # PROMOTION PIECE TYPE FLAG
                            encoded_move[4:10] = target_square
                            encoded_move[10:16] = origin_square
                            encoded_pawn_moves.append(encoded_move)
            ...
            for pos_idx in self.black_pawns.search(1):
                ...
                if not self.rank["1"][pos_idx]:
                    ...
                    # CAPTURE MOVE LOGIC
                    if (
                        not self.file["a"][pos_idx] and self.white_pieces[pos_idx + 7]
                    ):  # LEFT CAPTURE
                        target_squares.append(
                            pos_idx_to_bitarray(pos_idx + 7, length=6)
                        )
                    if (
                        not self.file["h"][pos_idx] and self.white_pieces[pos_idx + 9]
                    ):  # RIGHT CAPTURE
                        target_squares.append(
                            pos_idx_to_bitarray(pos_idx + 9, length=6)
                        )
