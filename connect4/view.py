...
class C4Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        for i in range(1, 9):
            ...
            arcade.draw_line(x_position, BORDER, x_position, SCREEN_HEIGHT-BORDER, arcade.color.BLACK, 2)
        ...
        if self.board.red_move and self.result == 2:
            ...
            self.board = self.board.make_move((self.selected, self.board.bottom_available(self.selected)))
            self.update_board(self.board)
            self.selected = -1
        ...
        if self.result != 2:
            ...
            arcade.draw_text(text, 450, 675, arcade.color.BLACK, 15, anchor_x="center", anchor_y="center")
