...
def training_loop():
    ...
    for i in range(13, MAX_ITERATIONS):
        ...
        score, res = parallel_parallel_evaluation(GAMES_TO_EVAL, MCTS_ITER, i, net, old_net, fp, old_exists)
...
def parallel_parallel_evaluation(GAMES_TO_EVAL, MCTS_ITER, i, net, old_net, fp, old_exists):
    score = 0
    res = [0] * 3
    ...
    # )
    process = Process(
        target=parallel_parallel_eval_wrapper2, args=(queue, stop_signal, GAMES_TO_EVAL, MCTS_ITER, fp, old_exists)
    )
    process.start()
...
def parallel_parallel_eval_wrapper2(queue, stop_sig, GAMES_TO_EVAL, MCTS_ITER, fp, old_exists):
    net = C4Net().to(device)
